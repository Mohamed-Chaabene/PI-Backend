...
import t.esprit.arctic.jobmatch.service.RecommendationEventService;
...
public class EvenementController {
    ...
    private final RecommendationEventService   recommendationService;
    ...
    public ResponseEntity<List<EvenementResponse>> getByOrganisateur(
    ...
    public ResponseEntity<EvenementStatsResponse> getStats(
        ...
        return ResponseEntity.ok(
                service.getStats(mois, annee, organisateurId));
    }


    @GetMapping("/export-ics/confirmed/{candidatId}")
    public ResponseEntity<byte[]> exportConfirmed(
            @PathVariable Long candidatId) {

        List<Participation> participations =
                participationService.findConfirmedByCandidatId(
                        candidatId, "CONFIRME");
        ...
        for (Participation p : participations) {
            ...
            ics.append("BEGIN:VEVENT\r\n")
                    ...
                    .append("DESCRIPTION:")
                    .append(ev.getType() != null ? ev.getType() : "")
                    .append("\r\n")
                    .append("LOCATION:")
                    .append(ev.getLieu() != null ? ev.getLieu() : "")
                    .append("\r\n")
                    .append("DTSTART:")
                    .append(formatIcsDate(ev.getDateHeure()))
                    .append("\r\n")
                    .append("DTEND:")
                    .append(formatIcsDate(ev.getDateHeure().plusHours(2)))
                    .append("\r\n")
                    .append("END:VEVENT\r\n");
        }
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION,
                        "attachment; filename=\"mes-evenements-confirmes.ics\"")
    ...
    @GetMapping(value = "/recommended/{candidateId}",
            produces = "application/json")
    public ResponseEntity<List<Evenement>> getRecommended(
            @PathVariable Long candidateId) {

        // Charger le candidat depuis la BDD
        Candidat candidat = (Candidat) utilisateurRepository
                .findById(candidateId)
                .orElseThrow(() ->
                        new RuntimeException(
                                "Candidat non trouvé : " + candidateId));

        // RecommendationService normalise et appelle Flask
        List<Evenement> events = recommendationService
                .getRecommendedEvents(candidateId, candidat);
