...
public class AuthController {
    ...
    public RegisterResponse register(@RequestBody RegisterRequest request) {
        ...
        System.out.println("[EMAIL VERIFICATION] token for " + savedUser.getEmail() + ": " + savedUser.getEmailVerificationToken());
    ...
    public LoginResponse login(@RequestBody LoginRequest request) {
        try {
            ...
            Utilisateur currentUser = service.getByEmail(request.email);
    ...
    public java.util.Map<String, Object> verifyEmail(@RequestParam String token) {
        boolean verified = service.verifyEmailToken(token);
        return java.util.Map.of(
                "verified", verified,
                "message", verified ? "Email vérifié avec succès" : "Lien de vérification invalide ou expiré"
        );
    }

    @PostMapping("/verification/resend")
    public java.util.Map<String, Object> resendVerification(@RequestBody java.util.Map<String, String> body) {
        String email = body.get("email");
        String token = service.resendVerification(email);
        System.out.println("[EMAIL VERIFICATION] resend token for " + email + ": " + token);
        return java.util.Map.of("message", "Lien de vérification renvoyé");
    }

    @GetMapping("/verification/status")
    public java.util.Map<String, Object> verificationStatus(Authentication auth) {
        Utilisateur user = service.getByEmail(auth.getName());
        return java.util.Map.of(
                "emailVerified", user.isEmailVerified(),
                "identityStatus", user.getIdentityVerificationStatus().name()
        );
    }

    @PostMapping("/verification/identity")
    public java.util.Map<String, Object> requestIdentityVerification(Authentication auth) {
        Utilisateur user = service.getByEmail(auth.getName());
        if (user.getIdentityVerificationStatus() == IdentityVerificationStatus.UNVERIFIED) {
            user.setIdentityVerificationStatus(IdentityVerificationStatus.PENDING);
            service.updateIdentityStatus(user.getId(), IdentityVerificationStatus.PENDING);
        }
        return java.util.Map.of("status", user.getIdentityVerificationStatus().name());
    }
}